...
do helpc=1,ns
	do helpcc=1,ni
		valport(helpc,helpcc)=(1.0/R(tc))*probs(tc,sc,helpc)*pi(tc+1,helpcc)*ah(helpc,helpcc) 
	end do
end do
...
do hc=1,hsc-1
	do hhc=1,ni
		call basefun (grida(tc+1,tyc,hc,hhc,:),pa,ah(hc,hhc),vals,inds)
		npla(hc,hhc)=vals(1)*lafun(tc+1,tyc,hc,hhc,inds(1))+vals(2)*lafun(tc+1,tyc,hc,hhc,inds(2))
		erhs((hc-1)*ni+hhc)=npla(hc,hhc)
	end do
end do
do hhc=1,hic
	call basefun (grida(tc+1,tyc,hsc,hhc,:),pa,ah(hsc,hhc),vals,inds)
	npla(hsc,hhc)=vals(1)*lafun(tc+1,tyc,hsc,hhc,inds(1))+vals(2)*lafun(tc+1,tyc,hsc,hhc,inds(2))
